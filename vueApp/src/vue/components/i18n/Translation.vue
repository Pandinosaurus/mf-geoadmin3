export class Lang {
  ...
  _determineFallbackLanguage() {
    let gaGlobalOptions_languages = ['de', 'en', 'it', 'fr', 'rm'];
    let gaGlobalOptions_translationFallbackCode = 'de';
    let lang = ($window.navigator.userLanguage ||
    $window.navigator.language).split('-')[0];

    if (gaGlobalOptions_languages.indexOf(lang) === -1) {
